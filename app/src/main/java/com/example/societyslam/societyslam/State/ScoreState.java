...
public class ScoreState extends State {
    ...
    public void render(Painter g) {
        if(SettingsState.getCurrentLanguage() == "English"){
            ...
            g.drawString("Highest game score ", 200, 35, Color.WHITE);
            g.setFont(Typeface.DEFAULT, 30);
            g.drawString("Player ", 200, 100, Color.WHITE);
            g.drawString(winnersName, 200, 140, Color.WHITE);
            g.drawString("Score ", 400, 100, Color.WHITE);
            g.drawString(highScore, 430, 140, Color.WHITE);
        } else if(SettingsState.getCurrentLanguage() == "Polish"){
            g.drawImage(Assets.coinTossBackground, 0, 0);
            backArrowButton.render(g);
            g.setFont(Typeface.DEFAULT, 40);
            g.setColor(Color.WHITE);
            g.drawString("Najwyższy wynik gry ", 200, 35, Color.WHITE);
            g.setFont(Typeface.DEFAULT, 30);
            g.drawString("Gracz ", 200, 100, Color.WHITE);
            g.drawString(winnersName, 200, 140, Color.WHITE);
            g.drawString("Wynik", 400, 100, Color.WHITE);
            g.drawString(highScore, 430, 140, Color.WHITE);
        }
